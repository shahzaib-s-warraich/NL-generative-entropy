...
dataset = load_dataset("wmt/wmt19", "de-en", split="validation").select(range(25))
for data in tqdm(dataset):
    input_text = data['translation']['de']
    input_text = '[INST] Instruction: Translate the following text from German to English ... Text: ' + input_text + ' Translation: [/INST]'
    ...
    with torch.no_grad():
        base_sample = model.generate(input_ids, attention_mask=attention_mask, max_length=75, temperature=1.5, top_p=0.9, do_sample=True)
        # base_sample = model.generate(input_ids, attention_mask=attention_mask, max_length=75)
    ...
    for i in range(100): # n auxiliary samples
        ...
        torch.manual_seed(np.random.randint(0, 10000))
        with torch.no_grad():
            aux_sample = model.generate(input_ids, attention_mask=attention_mask, max_length=75, temperature=1.5, top_p=0.9, do_sample=True)
            # aux_sample = model.generate(input_ids, attention_mask=attention_mask, max_length=75)
...
with open("transduction.json", "w") as outfile:
    outfile.write(out)